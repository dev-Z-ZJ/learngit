...
public class UsersAction extends BaseAction {
	...
	//设置默认地址
	@RequestMapping("addUaddress.action")
	public String addUAddress() {
		String username = (String) this.getSession().getAttribute("username");
		String uaddress = (String) this.getSession().getAttribute("uaddress");
		System.out.println("address = " + uaddress);
		System.out.println(username);
		Users user = new Users();
		user.setUsername(username);
		List<Users> list = this.usersService.getUsersByCond(user);
		user = list.get(0);
